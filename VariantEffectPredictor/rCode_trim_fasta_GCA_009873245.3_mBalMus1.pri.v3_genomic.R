inFastaNamePrefix <- "GCA_009873245.3_mBalMus1.pri.v3_genomic"
thisFasta <- readLines(paste(inFastaNamePrefix,".fna",sep=""))
...
for(i in 1:length(nameIndex)){
  ...
  if(sum(NLocs >= seqStarts[i] & NLocs <= seqEnds[i]) > 0){
    ...
    if(gregexpr("N",thisInScaf)[[1]][1] != -1){
      ...
      if(sum(theseNLocs[1:(length(theseNLocs)-1)] != theseNLocs[2:(length(theseNLocs))] -1) > 0){
        ...
        if(sum(theseNLocs[2:length(theseNLocs)] != theseNLocs[1:(length(theseNLocs)-1)] +1) > 0)
        lastNLocs<- c(lastNLocs,theseNLocs[length(theseNLocs)])
      }
