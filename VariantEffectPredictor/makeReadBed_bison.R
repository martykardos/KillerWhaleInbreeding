...
chroms <- read.table("chromSizes.Bison_UMD1.0_HiC_pruned.fasta",header=FALSE)
readCount <- sum(chroms[,2])/10
readBed <- matrix(NA,nrow=round(readCount + readCount),ncol=3)
...
write.table(readBed,file="Bison_UMD1.0_HiC_pruned_read.bed",quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
